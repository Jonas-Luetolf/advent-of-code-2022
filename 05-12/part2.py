...
for x in range(0,len(inp_field[0]),4):
    field.append(list([inp_field[y][x:x+3] for y in range(0,len(inp_field)) if inp_field[y][x:x+3] != "   "]))

#format steps
steps=steps.replace("move","").replace("from","").replace("to","").split("\n")
steps=[i.split(" ") for i in steps]
steps=list(map(lambda step: list(map(int,list(filter(lambda i: i != "", step)))),steps))

#move crates
for i in steps:
    to_move=field[i[1]-1][:i[0]]
    field[i[2]-1]=list(to_move)+field[i[2]-1]
